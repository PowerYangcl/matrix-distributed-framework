...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports) {
    ...
    , TPL_HEADER = function(options) {
        ...
        options = options || {};
        return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.cols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}',
            function() {
    ...
    , TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
        ...
        , '<div class="layui-table-box">', '{{# if(d.data.loading){ }}', '<div class="layui-table-init" style="background-color: #fff;">', '<i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>', '</div>', '{{# } }}'
        ...
        , '<style>', '{{# layui.each(d.data.cols, function(i1, item1){', 'layui.each(item1, function(i2, item2){ }}', '.laytable-cell-{{d.index}}-{{i1}}-{{i2}}{ ', '{{# if(item2.width){ }}', 'width: {{item2.width}}px;', '{{# } }}', ' }', '{{# });', '}); }}', '</style>', '</div>'
    ].join('')

    , _WIN = $(window), _DOC = $(document)
    ...
    Class.prototype.render = function() {
        ...
        options.response = $.extend({
            statusName: 'code',
            statusCode: 200,   // 300 接口请求成功，但是没有返回数据；200 接口请求成功同时返回列表数据。
            msgName: 'msg',
            dataName: 'data',
            countName: 'count'
        }, options.response);
    ...
    Class.prototype.setParentCol = function(hide, parentKey) {
        ...
        var parentTh = that.layHeader.find('th[data-key="' + options.index + '-' + parentKey + '"]') //获取父列元素
        var parentColspan = parseInt(parentTh.attr('colspan')) || 0;

        if (parentTh[0]) {
            var arrParentKey = parentKey.split('-');
            var getThisCol = options.cols[arrParentKey[0]][arrParentKey[1]];
...
//        }
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (typeof options.parseData === 'function') { // 如果有数据解析的回调，则获得其返回的数据
                        res = options.parseData(res) || res;
                    }
                    
                    if(res.status == "success"){
                    	if (res.code != response.statusCode) {  // 默认值为200|300 接口请求成功，但是没有返回数据；200 接口请求成功同时返回列表数据。
                            that.renderForm();
                            that.errorView( res.msg || ('没有查询到可以显示的数据'));
                        } else {
                            that.renderData(res, curr, res[response.countName]);
                            sort();
                            options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
                            console.log("接口请求+视图渲染耗时:  " + options.time + " | 接口地址：" + options.url + " | 参数：" + JSON.stringify(data));
                        }
                        that.setColsWidth();
            ...
            res[response.dataName] = options.data.list.concat().splice(startLimit, options.limit);
            res[response.countName] = options.data.list.length;
    ...
    Class.prototype.renderData = function(res, curr, count_, sort) {
        ...
        var render = function() { 		// 后续性能提升的重点
            ...
            layui.each(data.list , function(i1, item1) {
                var tds = [],
                    tds_fixed = [],
                    tds_fixed_r = [],
        ...
        table.cache[that.key] = data.list; // 记录数据
        table.cache[that.key + '-sort'] = res;        // 为字段排序记录数据

        //显示隐藏分页栏
        that.layPage[(count == 0 || (data.list.length === 0 && curr == 1)) ? 'addClass' : 'removeClass'](HIDE);
        ...
        render(); //渲染数据
        that.renderTotal(data.list); //数据合计
    ...
    //数据合计行
    Class.prototype.renderTotal = function(list) {
        var that = this;
        var options = that.config;
        var totalNums = {};
        ...
        layui.each(list , function(i1, item1) {
            if (item1.length === 0) 
            	return;
            that.eachCols(function(i3, item3) {
                var field = item3.field || i3;
        ...
        that.eachCols(function(i3, item3) {
            ...
            var content = function() {
                    ...
                    var thisTotalNum = parseFloat(totalNums[field]).toFixed(2);
                    var tplData = {};
    ...
    Class.prototype.sort = function(th, type, pull, formEvent) {
        var that = this;
        var field;
        var res = table.cache[that.key + '-sort'];
        var options = that.config;
        var filter = options.elem.attr('lay-filter');
        var data = table.cache[that.key];   // a list object - Yangcl
        var thisData;
        ...
        res[options.response.dataName].list = thisData || data;
                ...
                case 'LAYTABLE_EXPORT': //导出
                    if (device.ie) {
                        ...
                        openPanel({
                            list: function() {
                                return [
                                    '<li data-type="csv">导出到 Csv 文件</li>', '<li data-type="xls">导出到 Excel 文件</li>'
                                ].join('')
                            }(),
        ...
        that.layBody.on('click', '.' + ELEM_GRID_DOWN, function(e) {
            ...
            that.tipsIndex = layer.tips([
                '<div class="layui-table-tips-main" style="margin-top: -' + (elemCell.height() + 16) + 'px;' + function() {
                    ...
                    return '';
                }() + '">', elemCell.html(), '</div>', '<i class="layui-icon layui-table-tips-c layui-icon-close"></i>'
            ].join(''), elemCell[0], {
                tips: [3, ''],
                time: -1,
                anim: -1,
    ...
    table.init = function(filter, settings) {
        ...
        elemTable.each(function() {
            ...
            othis.find('tbody>tr').each(function(i1) {
                ...
                options.data[i1] = row;
                console.log("table.js | 表格初始化重载 | table.init line=1726 | 如果输出这段日志请到响应位置删除对应代码" + JSON.stringify(options));
            });
