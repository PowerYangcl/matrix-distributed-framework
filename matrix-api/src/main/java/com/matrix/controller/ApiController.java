...
import com.matrix.service.impl.TaskApiService;

/**
 * @description: api入口控制器|每个Request请求会在Tomcat线程池中创建一个ApiController对象
 *
 * @author Yangcl
...
public class ApiController  extends BaseController{
	...
	private static ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("Task-Api-Service-线程池-%s").build();
	...
	public JSONObject apiService(HttpServletRequest request , HttpServletResponse response , HttpSession session , String json){ 
		return this.apiServiceTask(request , response , session , json);
	}
	 ...
	 * @version 1.0.0.1
	 */
	private  JSONObject apiServiceTask(HttpServletRequest request, HttpServletResponse response , HttpSession session , String json) {
