 ...
 * 最长回文字串
 * 
 * 返回给定字符串的最长回文字串, manacher算法，采用动态规划思想 每次根据当前检查的字符array[i]的回文半径radius[i]，
...
public class HuiwenString {
	...
	public static void manacher(char[] s) {
		...
		int maxLen = 0;
		while (i < s.length) {
			while (i - j - 1 > -1 && i + j + 1 < s.length && s[i - j - 1] == s[i + j + 1]) {     // 扫描得出rad值
				j++;
			}
			rad[i] = j;
			...
			i = i + k;  // update the next search center
			j = Math.max(j - k, 0);  // update the next search radius, if always 0, duplicate work is done
		}
